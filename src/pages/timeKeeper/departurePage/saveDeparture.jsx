...
export default function SaveDeparture({ modelStatus, detail }) {
  ...
  const onSubmit = (data) => {
    ...
    if (!detail) {
      axios
        .post("http://localhost:3000/timekeeping/departure", data)
        .then(function (response) {
        ...
        .catch((e) => {
          alert("Departure Save Failed");
          setSubmitLoader(false);
        });
    } else {
      axios.put(
        "http://localhost:3000/timekeeping/departure/" + detail?.id,
        data
          .then(function (response) {
            if (response?.data?.status == "success") {
              alert("Departure Saved Successfully");
